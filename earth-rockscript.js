...
// --- 宣告一個全域變數，用來存放手機拖曳時的「視覺分身」 ---
let touchClone = null;

function setupCards() {
    document.querySelectorAll('.force-card').forEach(card => {
        // ==========================================
        // 1. 電腦版與共用邏輯：點擊、懸停與原生拖曳
        // ==========================================
        card.onclick = () => {
            // 如果卡片已經被分類完了，就不做反應
            if (card.parentElement.id === 'zone-nature' || card.parentElement.id === 'zone-human') return;
        ...
        card.onmouseleave = () => {
            if (selectedForceItem) {
                showDescription(selectedForceItem.getAttribute('data-desc'));
            } else {
                clearDescription();
            }
        };
        ...
        card.addEventListener('touchstart', (e) => {
            ...
            touchClone.id = "touch-clone-card";
            // 套用 Tailwind 類別：讓分身浮在最上層、稍微透明、加點陰影，並設定為 fixed 定位
            touchClone.className = "force-card bg-white px-3 py-2 rounded-lg font-bold border-2 border-blue-500 shadow-xl text-center text-sm fixed z-[100] pointer-events-none opacity-90 transform scale-105";
        ...
        }, { passive: false }); // 必須設為 false 才能使用 preventDefault
        ...
        card.addEventListener('touchend', (e) => {
            ...
            // 2. 像X光一樣，穿透座標找出手指放開位置「底下」的元素是什麼
            const dropTarget = document.elementFromPoint(touch.clientX, touch.clientY);
            
            if (dropTarget) {
                // 3. 檢查放開的位置底下，有沒有包含我們的分類框框？
                // 利用您原本寫好的 ondrop 屬性來精準判定
                if (dropTarget.closest('[ondrop*="nature"]')) {
                    placeForce('nature');
                } else if (dropTarget.closest('[ondrop*="human"]')) {
                    placeForce('human');
                }
                // 如果沒有放在框框內，卡片會自動彈回（因為分身消失，本體還在原地），並保留黃框選取狀態
            }
        });
    });
}
